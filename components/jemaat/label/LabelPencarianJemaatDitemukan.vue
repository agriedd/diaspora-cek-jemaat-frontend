	...
	<div class="text-sm rounded-xl border p-8 max-w-md w-full
		...
		<div class="flex flex-col gap-4">
			<div class="flex gap-4">
				<div class="rounded-2xl h-16 w-16 bg-sky-600 shrink-0 flex
				...
				<div class="grow flex flex-col gap-0 justify-center">
					<div class="rounded-md line-clamp-1 font-medium">
						{{ item.nama }}
					</div>
					<div class="rounded-md line-clamp-3 text-xs" v-if="item.rayon">
						Jemaat terdaftar pada {{ item.rayon }}
					</div>
					<div class="rounded-md line-clamp-3 text-xs" v-else>
						Jemaat terdaftar pada system tanpa informasi rayon
					</div>
				</div>
			</div>
